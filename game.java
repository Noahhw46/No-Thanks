...
public class game {
    ...
    public static void testStuff() {
        ...
        ArrayList<Player> playerTest = new ArrayList<Player>();
        ArrayList<Integer> removedCardsTest = new ArrayList<Integer>();
        ArrayList<Integer> deckTest = new ArrayList<Integer>();
        ...
        System.out.println("Noah's Score is: ");
        System.out.println(Noah.calcScore(Noah.getCards(), Noah.getChips()));
        System.out.println("Noah's Score should be: " + (19 + 3 + 5 - 6));

        System.out.println("Gavin's Score is: ");
        System.out.println(Gavin.calcScore(Gavin.getCards(), Gavin.getChips()));
        System.out.println("Gavin's Score should be: " + (1 + 29 + 18 + 4 - 5));

        System.out.println("Max's's Score is: ");
        System.out.println(Max.calcScore(Max.getCards(), Max.getChips()));
        System.out.println("Max's Score should be: " + (1 + 4 + 8 + 12 + 33 - 10));
        System.out.println(Max.getScore());

        while (removedCardsTest.size() < 9) {
            int random = (int) ((Math.random() * 33) + 3);
            if (!removedCardsTest.contains(random)) {
                removedCardsTest.add(random);
            }
        }
        Collections.sort(removedCardsTest);

        for (int i = 0; i < deckTest.size() - 1; i++) {
            for (int j = 0; j < removedCardsTest.size() - 1; j++) {
                if (deckTest.get(i) == removedCardsTest.get(j)) {
                    deckTest.remove(i);
                }
            }
        }
